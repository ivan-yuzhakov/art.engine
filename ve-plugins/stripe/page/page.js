var plugin_stripe_page = {
	arr: {},
	mode: false,
	el: {
		parent: $('#plugin_stripe_page')
	},
	...
	loadList: function(callback)
	{
		...
		var url = '?plugins/stripe/page/get_list', data = {section: x.active_section};

		$.post(url, data, function(json){
			if (json.status === 'DISABLE_PLUGIN') {
				x.error(json);
			}
			if (json.status === 'EMPTY_PUBLIC_KEY') {
				x.error(json);
			}
			if (json.status === 'EMPTY_PRIVATE_KEY') {
				x.error(json);
			}
	...
	error: function(response)
	{
		...
		if (response.status === 'EMPTY_PUBLIC_KEY') {
			x.el.wrapper.html('<div class="error">' + x.lang.error_public_key_empty + '</div>');
		}

		if (response.status === 'EMPTY_PRIVATE_KEY') {
			x.el.wrapper.html('<div class="error">' + x.lang.error_private_key_empty + '</div>');
		}
