var database = {
	...
	draw_items: function(callback)
	{
		...
		var load_items_section = false;
		$.each(x.config.display, function(i, id){
			if (fields.arr.fields[id] && fields.arr.fields[id].type === 'items') load_items_section = true;
		});
		if (load_items_section && (!window.items || Object.keys(window.items.arr).length === 0)) {
